  ...
  const renderEditForm = (mapping: Mapping) => (
    <div className="p-4 border rounded-lg bg-gray-50 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <GuideOverlay
          ...
          <select
            className={selectClasses}
            value={mapping.type}
            onChange={e => setEditingMapping({ ...mapping, type: e.target.value })}>
            {TYPE_OPTIONS.map(o => (
      ...
      <div className="flex justify-end space-x-3">
        <button onClick={() => setEditingMapping(null)} className="text-sm font-medium text-gray-600">
          Cancel
        </button>
        <GuideOverlay
            ...
            <div key={m.id} className="bg-white border border-gray-200 rounded-lg">
              ...
              {isExpanded && (
                <div className="px-5 py-4 bg-gray-50 border border-gray-100 rounded-b-lg shadow-sm text-sm text-gray-800 space-y-2 flex flex-col items-start">
                  <div>
                    <span className="font-semibold text-gray-900">URI:</span> {m.uri}
                  </div>
                  <div>
                    <span className="font-semibold text-gray-900">Country:</span> {m.location_country}
                  </div>
                  <div>
                    <span className="font-semibold text-gray-900">City:</span>{" "}
                    {Array.isArray(m.location_city) ? m.location_city.join(", ") : m.location_city}
                  </div>
                </div>
              )}
            </div>

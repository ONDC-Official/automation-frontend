...
export const URIsSection: React.FC<URIsSectionProps> = ({ uris, onAddUri, onEditUri, onDeleteUri }) => {
  const handleAdd = () => {
      ...
      <div className="mt-4 space-y-3">
        {uris.map(uri => (
          <div
            ...
            <div>
              <span className="font-mono text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">ID: {uri.id}</span>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => handleDelete(uri.id)} className="text-gray-400 hover:text-red-600">
                <TrashIcon />
              </button>
            </div>
          </div>

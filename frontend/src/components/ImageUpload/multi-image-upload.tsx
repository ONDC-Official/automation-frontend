...
const MultiImageUpload: React.FC<MultiImageUploadProps> = ({
    ...
    try {
      ...
      if (response.data.success) {
        const newUploadedUrls = response.data.data.images.map(
          (img: { imageUrl?: string }) => img.imageUrl || defaultImageUrl
        );
  ...
  const removeAll = () => {
    ...
    const fileInput = document.querySelector(`input[name="${folder}-images"]`) as HTMLInputElement;
    if (fileInput) {
      fileInput.value = "";
    }
  };
      ...
      <div className="space-y-4">
        ...
        {selectedFiles.length > 0 && (
          <div className="flex items-center space-x-2 p-2 bg-green-50 rounded-md border border-green-200">
            ...
            <Button type="text" size="small" onClick={removeAll} icon={<DeleteOutlined />} danger>
              Clear
            </Button>
          </div>
        )}

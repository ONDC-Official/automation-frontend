...
type Props = {
  ...
  HtmlFormConfigInFlow: IFormFieldConfigProps;
};
...
export default function ProtocolHTMLForm({
  ...
  const formHtml = useMemo<string>(() => {
    return (
      jsonpath.query({ reference_data: referenceData }, HtmlFormConfigInFlow.reference || "")[0] ||
      ""
    );
  ...
  const setField = (name: string, val: unknown) => {
    setValues((prev) => ({ ...prev, [name]: val as ValueState[string] }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (hasFile) {
        ...
        // Convert FormData to Record<string, unknown> for htmlFormSubmit
        const formDataRecord: Record<string, unknown> = {};
        for (const [key, value] of fd.entries()) {
          if (formDataRecord[key]) {
            // If key already exists, convert to array
            const existing = formDataRecord[key];
            if (Array.isArray(existing)) {
              existing.push(value);
            } else {
              formDataRecord[key] = [existing, value];
            }
          } else {
        ...
        // Convert URLSearchParams to Record<string, unknown> for htmlFormSubmit
      ...
      // Type guard for response data structure
      ...
      function isSubmissionResponse(obj: unknown): obj is SubmissionResponse {
        return (
          typeof obj === "object" &&
          obj !== null &&
          ("submission_id" in obj || "data" in obj || "result" in obj)
        );
      }

      const responseData = isSubmissionResponse(data) ? data : null;
      const finalSubmissionId =
        responseData?.submission_id ??
        responseData?.data?.submission_id ??
        responseData?.result?.submission_id ??
        "";
      ...
      for (const f of parsed.fields) {
        ...
        if (f.kind === "checkbox-group" || (f.kind === "select" && f.multiple)) {
          ...
          if (Array.isArray(val)) {
            val.forEach(
              (file, i) => (plainPayload[`${f.name}[${i}]`] = (file as File)?.name ?? "")
            );
      ...
      case "textlike": {
        ...
        return labelEl(
          <input
            ...
            required={f.required}
            disabled={f.disabled}
            min={tf.min as string | number | undefined}
            max={tf.max as string | number | undefined}
            step={tf.step as string | number | undefined}
            pattern={tf.pattern}
      ...
      case "select": {
        ...
        return labelEl(
          <select
            name={f.name}
            value={
              (sel.multiple ? ((v as string[]) ?? []) : ((v as string) ?? "")) as string | string[]
            }
            onChange={(e) => {
              if (sel.multiple) {

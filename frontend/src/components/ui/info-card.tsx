    ...
    <div className="w-full bg-gray-100 backdrop-blur-md rounded-md border border-gray-200 p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(data).map(([key, value], index) => (
          ...
          >
            <span className="text-sm font-extrabold text-sky-700 whitespace-nowrap">{key}</span>
            {/* I've added the robust truncation from before, as it's still a good idea! */}
            <span className="text-sm text-gray-800 font-medium ml-2 truncate">{value}</span>
          </div>
        ))}
      </div>
    </div>

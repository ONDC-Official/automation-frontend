...
export default function RideMapTab({ flowId }: { flowId: string | null }) {
    ...
    if (!flowId || !transactionId) {
        return (
            <EmptyState variant="text" message="Start a ride-hailing flow to view its map here." />
        );
    }
    ...
    if (!rideMap.isTracking || trackingReset) {
        ...
        return (
            <EmptyState
                variant="text"
                message="The ride map appears once you enter the pickup & destination locations."
            />
        );
    }
                ...
                {isController && !finished && (
                    <div className="flex items-center gap-2">
                        <Button
                            ...
                            className={
                                autoRunActive
                                    ? "rounded-full border-amber-400 bg-amber-50 text-amber-700 hover:bg-amber-50"
                                    : "rounded-full border-sky-300 bg-sky-50 text-sky-700 hover:bg-sky-100"
                            }
                        >
                        ...
                        <span className="flex items-center gap-0.5 text-xs text-gray-500">
                            {[1, 2, 4].map((s) => (
                                <Button
                                    ...
                                    className={
                                        speed === s
                                            ? "rounded bg-sky-600 text-white hover:bg-sky-600"

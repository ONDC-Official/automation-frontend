...
const difficultyOptions: DifficultyOption[] = [
  {
    ...
    description: "Determines if the flow should stop after the first negative acknowledgment.",
    options: [
...
const DifficultyForm = ({ submitFunction, subUrl }: { submitFunction: () => Promise<void>; subUrl: string }) => {
  ...
  useEffect(() => {
    ...
    difficultyOptions.forEach(option => {
      const selectedOption = option.options.find(opt => opt.value === selections[option.id]);
      if (selectedOption) {
        totalPoints += selectedOption.points;
      }
      const maxOption = option.options.reduce((prev, current) => (prev.points > current.points ? prev : current));
      maxPoints += maxOption.points;
    });

    const difficultyPercentage = maxPoints > 0 ? Math.round((totalPoints / maxPoints) * 100) : 0;
    ...
    <div className=" mt-2 p-2">
      <form onSubmit={handleSubmit} className="space-y-2">
        {difficultyOptions.map(option => (
          <div key={option.id} className="flex flex-col">
            <label htmlFor={option.id} className="text-lg font-medium text-sky-700">
              {option.label}
            </label>
            ...
            <select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => handleChange(option.id, e.target.value)}
        ...
        <div className="mt-8">
          <label className="block text-lg font-medium text-gray-700 mb-2">Total Difficulty: {totalDifficulty}%</label>
      ...
      <div className="mt-4 p-4 bg-gray-100 rounded-md border border-gray-300">
        ...
        <ul className="list-disc list-inside text-sm text-gray-600 mt-2">
          <li>Beat all flows at minimum 50% difficulty to move from staging to pre-prod</li>
          <li>Beat all flows at minimum 100% difficulty to move from pre-prod to prod</li>
        </ul>
      </div>
    </div>
  );

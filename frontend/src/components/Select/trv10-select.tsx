...
export default function TRV10AddOnSelect({
    ...
    message?: {
      catalog?: {
        providers?: Array<{
          ...
          items?: Array<{
            id: string;
            add_ons?: Array<{ id: string; quantity?: { maximum?: { count?: number } } }>;
          }>;
        }>;
      };
    };
  ...
  const handlePaste = (parsedPayload: unknown) => {
    try {
      const payload = parsedPayload as PastePayload;
            ...
            (item: {
              ...
              results.push({
                ...
                addOns: (item.add_ons || []).map(
                  (addon: { id: string; quantity?: { maximum?: { count?: number } } }) => ({
                    id: addon.id,
  ...
  const renderSelectOrInput = (
    ...
    if (options.length === 0) {
      return (
        <input
          type="text"
          {...register(name as Path<FormData>)}
    ...
    return (
      <select
        ...
        onChange={(e) => {
          ...
          if (selectedOption) {
            setValue(`items.${index}.addOns` as Path<FormData>, []);
            setValue(
              `items.${index}.providerid` as Path<FormData>,
              selectedOption.providerid as never
            );
          }
        }}
        className={inputStyle}
  ...
  return (
    <div>
      {isPayloadEditorActive && (
        <PayloadEditor mode="modal" onAdd={handlePaste as (parsedPayload: unknown) => void} />
      )}
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="border p-3 rounded space-y-2">
            ...
            {(() => {
              ...
              const selectedExtracted = itemOptions.find((opt) => opt.itemid === currentItemId);
                  ...
                  <div className={fieldWrapperStyle}>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (
                          !prev.some((addon: { id: string; quantity: number }) => addon.id === val)
                        ) {
                          setValue(`items.${index}.addOns`, [...prev, { id: val, quantity: 1 }]);
                        }
                      }}
                      value=""
                    >
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      {selectedItems[index]?.addOns?.map(
                        (selectedAddOn: { id: string; quantity: number }) => {
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const updated = currentAddOns.map((a) =>
                                      a.id === selectedAddOn.id ? { ...a, quantity: newQty } : a
                                    );
                                    setValue(`items.${index}.addOns`, updated);
                                  }}

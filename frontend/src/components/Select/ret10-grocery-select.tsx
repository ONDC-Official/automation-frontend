...
interface GroceryFormData {
  ...
  items: Array<{ itemId: string; quantity: number; location: string }>;
  [key: string]:
    | string
    | string[]
    | Array<{ itemId: string; quantity: number; location: string }>
    | boolean
    | undefined;
}
...
export default function Ret10GrocerySelect({
  ...
  const { control, handleSubmit, watch, register } = useForm<GroceryFormData>({
    defaultValues: {
      provider: "",
      provider_location: [],
      location_gps: "",
  ...
  const handlePaste = (data: {
    ...
    try {
      ...
      if (provider) {
        setItemOptions((provider.items || []).map((i: { id: string }) => i.id));
        setLocationOptions((provider.locations || []).map((l: { id: string }) => l.id));
      }

      const offers = providers
        .flatMap((p: { offers?: Array<{ id: string }> }) => p.offers || [])
              ...
              return (
                <div className="flex flex-col gap-2">
                  {locations.map((loc: { id: string }) => (
                    <label key={loc.id} className="inline-flex gap-2 items-center">
                      <input
                        type="checkbox"
                        value={loc.id}
              ...
              {offerOptions.map((offerId) => (
                <label key={offerId} className="inline-flex gap-2 items-center">
                  <input
                    type="checkbox"
                    value={offerId}
                    {...register(`offers_${offerId}` as Path<GroceryFormData>)}
                    className="accent-blue-600"
                  />
                  {offerId}
                </label>
...
function validateFormData(data: GroceryFormData): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];

    ...
    <div className="space-y-4">
      ...
      {timingFields.map((field, timingIndex) => (
        ...
        >
          ...
          {watchTimings?.[timingIndex]?.dayFrom &&
            watchTimings?.[timingIndex]?.dayTo && (
              <div className="mt-3 text-sm text-gray-600">
                <span className="font-medium">Active:</span>{" "}
                {watchTimings[timingIndex].dayFrom} to{" "}
                {watchTimings[timingIndex].dayTo}
      ...
      <div className="bg-blue-50 p-3 rounded-lg">
        <p className="text-sm text-blue-800">
          <strong>Tip:</strong> Add multiple timings for different day ranges
          (e.g., weekdays vs weekends) or special hours.
        </p>
      </div>
    </div>
  );
...
const CustomMenuFormEnhanced = ({
  ...
  const [showCustomizationModal, setShowCustomizationModal] = useState<
    number | null
  >(null);
  const [editingGroup, setEditingGroup] = useState<CustomizationGroup | null>(
    null
  );
  ...
  const handleAddCustomizationGroup = (menuIndex: number) => {
    ...
    setValue(`menu.${menuIndex}.customizationGroups`, [
      ...currentGroups,
      newGroup,
    ]);
  ...
  const handleEditCustomizationGroup = (
    menuIndex: number,
    groupIndex: number
  ) => {
  ...
  const handleSaveCustomizationGroup = () => {
    if (showCustomizationModal !== null && editingGroup) {
      ...
      const currentGroups =
        watchMenu[showCustomizationModal]?.customizationGroups || [];
      ...
      setValue(
        `menu.${showCustomizationModal}.customizationGroups`,
        currentGroups
      );
  ...
  const handleRemoveCustomizationGroup = (
    menuIndex: number,
    groupIndex: number
  ) => {
    const currentGroups = watchMenu[menuIndex]?.customizationGroups || [];
      ...
      const formData = {
        menuItems: data.menu.map((item: any) => {
          ...
          return {
            ...
            timeFrom:
              convertTimeFormat(item.timeFrom) ||
              processedTimings[0]?.timeFrom ||
              "",
            timeTo:
              convertTimeFormat(item.timeTo) ||
              processedTimings[0]?.timeTo ||
              "",
            // New multiple availability timings
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-blue-700">
                    <p>
                      This step is optional. You can skip it now and add menu
                      items later. Click "Skip This Step" to proceed without
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      {(!watchMenu[index]?.customizationGroups ||
                        watchMenu[index]?.customizationGroups?.length ===
                          0) && (
                        <p className="text-sm text-gray-500 italic">
                          No customizations added yet. Add customization groups
                          for size options, toppings, add-ons, etc.
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input
            ...
            value={group.seq || 1}
            onChange={(e) =>
              onChange({ ...group, seq: parseInt(e.target.value) || 1 })
            }
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
            placeholder="1"
          />
          <p className="text-xs text-gray-500 mt-1">
            Order of this group (1 = first)
          </p>
        </div>
      </div>

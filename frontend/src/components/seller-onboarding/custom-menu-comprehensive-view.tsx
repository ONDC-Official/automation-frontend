          ...
          itemNode.children!.push({
            ...
            title: (
              <Space>
                ...
                <Tag color="purple">{group.id}: {group.name}</Tag>
              </Space>
            )
          });
              ...
              title: (
                <Space>
                  ...
                  <Tooltip title={`Parent: ${group.id}`}>
                    <Tag color="purple" className="text-xs">→ {group.id}</Tag>
                  </Tooltip>
                </Space>
              )
            });
                ...
                {Object.entries(categorizedItems).map(([category, items]) => (
                  <Card key={category} className="shadow-md">
                    <h4 className="text-lg font-semibold mb-4">
                      {category} 
                      <Tag color="blue" className="ml-2">Menu Category</Tag>
                    </h4>
                                ...
                                <React.Fragment key={group.id}>
                                  <div className="bg-white p-3 rounded-lg border-2 border-purple-300">
                                    ...
                                    <div className="text-xs text-gray-600 mb-2">
                                      ...
                                      {group.required && <Tag color="red" className="ml-1 text-xs">Required</Tag>}
                                    </div>
                                    <div className="space-y-1">
                                      {group.items.map(opt => (
                                        <div key={opt.id} className="text-xs bg-gray-100 px-2 py-1 rounded">
                                          {opt.name} 
                                          {parseFloat(opt.price) > 0 && <span className="text-gray-500"> (+₹{opt.price})</span>}
                                        </div>
                                      ))}
                                    </div>
                                  </div>

...
export default function UserProfile() {
  ...
  useEffect(() => {
    ...
    if (!user.userDetails) {
      navigate(ROUTES.HOME); // Redirect to home if not logged in
    }
  }, []);
  ...
  async function handleLogout() {
    try {
      ...
      navigate(ROUTES.HOME); // Redirect to home on error
    }
  }
            ...
            <div className="max-w-4xl mx-auto p-6 bg-white rounded-xl">
              <div className="flex justify-between items-start flex-col sm:flex-row sm:items-center">
                ...
                <div className="text-gray-700 space-y-1 items-start justify-items-start">
                  <p>
                    <strong>Login:</strong> {user.userDetails?.githubId || "N/A"}
                  </p>
                  <p>
                    <strong>Participant ID:</strong> {user.userDetails?.participantId || "N/A"}
                  </p>
                </div>

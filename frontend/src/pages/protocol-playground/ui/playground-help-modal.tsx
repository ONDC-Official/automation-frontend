    ...
    },
    {
        ...
        content: (
            <div className="space-y-4">
                ...
                <Callout type="warn">
                    Resetting a step also clears all saveData-dependent downstream steps. If a
                    later step references a value from the reset step, it will need to be re-run
                    too.
                </Callout>
            </div>
        ),
                ...
                <div className="space-y-3 text-sm">
                    <div className="border border-gray-200 rounded-lg p-3">
                        ...
                        <p className="text-gray-700">
                            ...
                            values as context. This is what gets sent / logged as the step's
                            output.
                        </p>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-3">
                    ...
                    <div className="border border-gray-200 rounded-lg p-3">
                        ...
                        <p className="text-gray-700 mt-1">
                            A JSON Schema (<code>{"{ jsonSchema: { ... } }"}</code>). When non-empty,
                            running this step opens a form so you can provide runtime values that
                            are passed into the generator.
                        </p>
                    </div>
                </div>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <Step n={2}>
                        ...
                        alias is auto-created and appears as{" "}
                        <strong>Tentative Save Data</strong> on the right.
                    </Step>
                    <Step n={3}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="border border-amber-200 rounded-lg p-4 bg-amber-50/40">
                        ...
                        <p className="text-gray-700 text-sm mb-3">
                            ...
                            <strong>Base64-encoded JS function</strong>. The mock-runner decodes
                            and calls it with the step's payload, using the return value as the
                            saved data.
                        </p>
                    ...
                    <div className="border border-violet-200 rounded-lg p-4 bg-violet-50/40">
                        ...
                        <p className="text-gray-700 text-sm mb-3">
                            Use when you want to <strong>collect values from multiple steps
                            into a single array</strong> instead of overwriting. Prefix the alias
                            with <code>APPEND#</code>; the real key name is everything after the
                            prefix.
                        </p>
                        <pre className="bg-gray-900 text-emerald-400 text-xs rounded-lg p-3 overflow-x-auto">
                        ...
                        <p className="text-gray-700 text-sm mt-2">
                            ...
                            side:{" "}
                            <code>{"APPEND#myKey → EVAL#<base64fn>"}</code>
                        </p>
                    </div>
                    ...
                    <div className="border border-rose-200 rounded-lg p-4 bg-rose-50/40">
                        ...
                        <div className="mt-3 space-y-1 text-sm text-gray-700">
                            ...
                            <ul className="list-disc list-inside space-y-1 text-gray-600">
                                <li>
                                    <code>key</code> —{" "}
                                    <strong>must follow the pattern</strong>{" "}
                                    ...
                                    current flow. The runner clones that step's mock code (generator,
                                    validator, saveData, etc.) as the template for the new step.
                                    <code>&lt;n&gt;</code> is an incrementing number to keep keys
                                    unique when the same base action is generated multiple times
                                    (e.g. <code>GENERATED#1#on_update</code>,{" "}
                                    <code>GENERATED#2#on_update</code>).
                                </li>
                                <li><code>type</code> — ONDC action name (e.g. <code>on_update</code>)</li>
                                <li><code>unsolicited</code> — boolean, whether this is an unsolicited call</li>
                                <li><code>pair</code> — the paired response action, or <code>null</code></li>
                                <li><code>owner</code> — <code>"BAP"</code> or <code>"BPP"</code></li>
                                <li><code>expect</code> — boolean, whether a response is expected</li>
                            </ul>
                        </div>
                        <Callout type="info">
                            <code>MORE_SEQUENCE</code> is evaluated during session data
                            compilation (before each step runs), so the expansion is visible in the
                            timeline before the new steps execute.
                        </Callout>
                    </div>
                </div>

                <Callout type="warn">
                    Deleting a Save Data alias resets the transaction history from that step
                    onward — downstream steps that depended on the alias need to be re-run.
                </Callout>
            </div>
        ),
    },
    {
        ...
        content: (
            <div className="space-y-4">
                ...
                <Callout type="info">
                    If a step fails mid-sequence, execution stops and an error toast tells you
                    which action_id failed. Fix the generator and re-run.
                </Callout>
    ...
    },
    {
        ...
        content: (
            <div className="space-y-4">
                ...
                <Callout type="tip">
                    The playground auto-saves your config to localStorage on every change. Your
                    work survives page refreshes. Use <strong>Download</strong> for explicit
                    checkpoints or to share with teammates.
                </Callout>
            </div>
        ),
    },
        ...
        content: (
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <Step n={2}>
                        Enter your <strong>Subscriber URL</strong> and your role (
                        <Badge>BAP</Badge> or <Badge>BPP</Badge>).
                    </Step>
                    <Step n={3}>
...
const FAQS: Faq[] = [
    ...
    },
    {
        ...
        a: (
            <p>
                ...
                it, and store <code>{"EVAL#<base64>"}</code> as the path. The mock-runner calls
                your function with the step payload and uses the return value.
            </p>
        ),
    },
    {
    ...
    },
    {
        ...
        a: (
            <div className="space-y-2 text-sm text-gray-700">
                <p>
                    Yes — using <Badge color="red">MORE_SEQUENCE</Badge>. In a{" "}
                    <code>getSave</code> function (via <Badge color="amber">EVAL#</Badge>), return{" "}
                ...
                <p>
                    ...
                    <code>GENERATED#&lt;n&gt;#&lt;action_id&gt;</code> where{" "}
                    <code>action_id</code> is an action that <strong>already exists</strong> in the
                    current flow. The runner clones that existing step's mock code as the template
                    for the new dynamically-inserted step. Use the incrementing{" "}
                    <code>&lt;n&gt;</code> to keep keys unique when the same base action is
    ...
    },
    {
        ...
        a: (
            <p>
                Use <strong>Download</strong> to export a <code>.json</code> file and send it —
                they can use <strong>Upload</strong> to load it. Alternatively, paste the config
                into a GitHub Gist and share the URL; opening{" "}
                <code>?gist=&lt;gist-url&gt;</code> loads it automatically.
            </p>
        ),
    },
    {
        ...
        a: (
            <p>
                <strong>Download</strong> saves the config exactly as-is (including your JS code
                and transaction history) — meant for saving your work. <strong>Export for
                Deployment</strong> runs all steps, embeds the generated payloads as{" "}
                <code>examples</code>, strips transaction history, and outputs YAML — meant as a
                deployment artifact.
            </p>
        ),
    },
        ...
        <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl flex flex-col w-full max-w-5xl h-[88vh] overflow-hidden">
                ...
                <div className="flex flex-1 min-h-0">
                    {/* Sidebar */}
                    <nav className="w-52 shrink-0 border-r border-gray-100 py-4 overflow-y-auto">

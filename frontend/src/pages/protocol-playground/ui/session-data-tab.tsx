...
export default function SessionDataTab() {
  ...
  const handleSave = () => {
    ...
    // Nothing to save
    if (!historyEntry.saved_info || Object.keys(historyEntry.saved_info).length === 0) {
      return;
    }
      ...
      <div className="flex flex-1 bg-gray-900 text-gray-100 min-h-0 p-4">
        ...
        <div className="w-1/2 p-6 overflow-auto bg-gray-850">
          <div className="flex justify-between items-center">
            <h2 className="text-xl mb-0 font-bold text-sky-400">Save Data ({saveDataLength})</h2>
            <button
              onClick={() => {
                setError("");
              ...
              {Object.entries(savedInfo).map(([alias, path]) => (
                ...
                >
                  <JsonPathInput
                    ...
                    error={error}
                    setError={setError}
                    handleAdd={({ oldAlias, currAlias, currPath }: handleAddParam) =>

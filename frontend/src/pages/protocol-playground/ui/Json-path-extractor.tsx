...
const renderValue = (
  ...
  if (selected.status) {
    if (selected.type === SelectedType.SavedInfo) {
      className += "bg-gray-500/30 text-gray-100 font-medium ring-1 ring-gray-500/50";
    } else if (selected.type === SelectedType.SaveData) {
      className += "bg-sky-500/30 text-sky-200 font-medium ring-1 ring-sky-500/50";
    }
  } else {
  ...
  return (
    <Tooltip
      title={
        <div className="text-xs p-2 rounded-sm bg-white border border-gray-300">
          <div className="font-semibold text-black mb-1">Path:</div>
          <div className="text-black font-mono">{path}</div>
          {isTruncated && (
            <>
              <div className="font-semibold text-black mt-2 mb-1">Full Value:</div>
              <div className="text-black">{stringValue}</div>
            </>
          )}
        </div>
      }
    ...
    >
      <span onClick={(e) => handleKeyClick(path, key, e)} className={className}>
        ...
        <span className="text-xs text-gray-500 font-mono flex-shrink-0">{getTypeBadge(value)}</span>
        <button
          onClick={handleCopy}
  ...
  return (
    <div>
      {Object.entries(obj).map(([key, value]) => {
        ...
        return (
          <div key={key} style={{ paddingLeft: `${indent}px` }} className="whitespace-nowrap">
              ...
              <div className="inline-flex items-baseline gap-2 min-w-0">
                <Tooltip
                  title={
                    <div className="text-xs p-2 rounded-sm bg-white border border-gray-300">
                      <div className="font-semibold text-black mb-1">Path:</div>
                      <div className="text-black font-mono">{newPath}</div>
                    </div>
                  }
                  mouseEnterDelay={0.5}
                  arrow={true}
                  open={!isObject && !isArray ? false : undefined}
                  placement="top"
                  overlayInnerStyle={{ maxWidth: "20rem" }}
                >
                  <span
                ...
                <span className="text-gray-600 font-mono text-sm flex-shrink-0">:</span>
                  ...
                  <>
                    {(Array.isArray(value) ? value : []).map((item: unknown, index: number) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-gray-600 text-xs mr-2">{index}:</span>
                          {renderValue(
                            item,
                            arrayPath,
                            `${key}_${index}`,
...
const JsonViewer: React.FC<JsonViewerProps> = ({ data, isSelected, handleKeyClick }) => {
          ...
          if (Array.isArray(value)) {
            value.forEach((item, index) => {
              ...
              if (typeof item === "object" && item !== null) {
                ...
                const itemMatches = String(item).toLowerCase().includes(searchLower);
                if (itemMatches) {
            ...
            searchObject(value as Record<string, unknown>, newPath);
          }
        }
      });
    };
  ...
  const collapseAll = () => {
    ...
    const collectPaths = (obj: Record<string, unknown>, path: string = "$") => {
      Object.entries(obj).forEach(([key, value]) => {
        const newPath = `${path}.${key}`;
        if (typeof value === "object" && value !== null) {
          ...
          if (Array.isArray(value)) {
            ...
            collectPaths(value as Record<string, unknown>, newPath);
          }
        }
      });
    };
  ...
  return (
    <div className="font-mono text-sm h-full flex flex-col">
      ...
      {searchTerm && matchingPaths.size === 0 && (
        <div className="text-center text-gray-500 py-8">No results found for "{searchTerm}"</div>
      )}
    </div>
  );
};

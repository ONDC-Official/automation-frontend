    ...
    <div className="w-[340px] bg-white rounded-2xl border-2 border-gray-100 shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="px-2 py-2 bg-gradient-to-b from-sky-50 to-white">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500 font-mono">/{action.config.api}</span>
            </div>
      ...
      <div className="px-2 py-2 space-y-1 bg-white">
        {[
          ...
          .filter(
            (field): field is { label: string; value: string; valueClass: string } =>
              typeof field === "object" && field !== null && "label" in field
          )
          .map((field, i) => (
            <div
              key={i}
            ...
            >
              ...
              <span className={`text-xs font-bold px-2 py-1 rounded-md border ${field.valueClass}`}>
                {field.value}
              </span>
            </div>
          ))}

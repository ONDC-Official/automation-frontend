    ...
    <div className="w-80 p-4 bg-white border border-gray-200 rounded-lg shadow-xl">
      ...
      <div className="pb-3 mb-3 border-b border-gray-100">
        <div className="flex items-center gap-2 mb-1">
          <span className={`px-2 py-0.5 rounded text-xs font-bold ${methodStyles[method]}`}>{method}</span>
          <span className="text-sm text-gray-500 font-mono">/{actionConfig.api}</span>
        </div>
        <h4 className="text-base font-semibold text-gray-900 font-mono">{actionConfig.action_id}</h4>
      </div>
      ...
      <div className="space-y-2.5 mb-3">
        <div className="flex items-start">
          ...
          <span className={`text-sm font-medium ${actionConfig.owner === "BPP" ? "text-purple-700" : "text-sky-700"}`}>
            {actionConfig.owner}
          </span>
        </div>

        <div className="flex items-start">
          <span className="text-xs font-medium text-gray-500 w-24 flex-shrink-0 pt-0.5">Unsolicited</span>
          <span className={`text-sm font-medium ${actionConfig.unsolicited ? "text-green-600" : "text-gray-600"}`}>
        ...
        {actionConfig.responseFor && actionConfig.responseFor !== "NONE" && (
          <div className="flex items-start">
            <span className="text-xs font-medium text-gray-500 w-24 flex-shrink-0 pt-0.5">Response For</span>
            <span className="text-sm font-medium text-indigo-700 font-mono">{actionConfig.responseFor}</span>
          </div>
        )}
      </div>
    ...
    <div className="h-11 px-6 flex items-center justify-between gap-4 bg-white border-b border-gray-200">
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-md">
          <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wide ${methodStyles[method]}`}>
            {method}
          </span>
        ...
        {/* Action ID */}
        <span className="text-sm font-semibold text-gray-900 font-mono">{actionConfig.action_id}</span>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <Tippy content={<span className="text-xs">Insert Before</span>} delay={500}>
          <div>
            <IconButton icon={<TbColumnInsertLeft size={15} />} onClick={onAddBeforeClick} color="gray" label="" />
          </div>
        </Tippy>
        <Tippy content={<span className="text-xs">Insert After</span>} delay={500}>
          <div>
            <IconButton icon={<TbColumnInsertRight size={15} />} onClick={onAddAfterClick} color="gray" label="" />
          </div>
        </Tippy>
        <Tippy content={<span className="text-xs">Edit</span>} delay={500}>
          <div>
            <IconButton icon={<MdEdit size={15} />} onClick={onEditActionClick} color="sky" label="" />
          </div>
        </Tippy>
        <Tippy content={<span className="text-xs">Delete</span>} delay={500}>
          <div>
            <IconButton icon={<IoMdTrash size={15} />} onClick={onDeleteClick} color="red" label="" />
          </div>
        </Tippy>
      </div>
    </div>

      ...
      domains.forEach((domain: Domain) => {
        if (domain.key === payloadWithContext?.context?.domain) {
          domain.version.forEach((ver: DomainVersion) => {
            if (
              ver.key ===
              (payloadWithContext?.context?.version || payloadWithContext?.context?.core_version)
            ) {
              isDomainActive = true;
            }
          });
    ...
    return (
      <div className="h-full flex items-center justify-center bg-gray-50 rounded-lg border border-gray-200">
        <div className="text-center p-8">
          ...
          <p className="text-gray-400 text-xs mt-1">Execute a function to see the output</p>
        </div>
      </div>
    );
  }
        ...
        {l2Result && (
          <div className="border-b border-gray-200">
            ...
            >
              ...
              {showL2Results ? (
                <IoChevronUp className={!l2Result.valid ? "text-red-400" : "text-green-400"} />
              ) : (
                <IoChevronDown className={!l2Result.valid ? "text-red-400" : "text-green-400"} />
              )}
            </button>
            {showL2Results && (

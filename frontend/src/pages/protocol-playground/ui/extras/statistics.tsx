...
export const CodeStatistics = ({ statistics, validation }: CodeStatisticsProps) => {
  if (!statistics) return null;

  const getComplexityColor = (complexity: number) => {
    if (complexity <= 5) return "text-sky-600 bg-sky-50 border-sky-200";
    if (complexity <= 10) return "text-yellow-600 bg-yellow-50 border-yellow-200";
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2 text-xs flex-wrap">
        <div className="px-2 py-1 bg-gray-100 rounded border border-gray-200">
          ...
          <span className="ml-1 font-semibold text-gray-900">{statistics.lines}</span>
        </div>
        <div className={`px-2 py-1 rounded border ${getComplexityColor(statistics.complexity)}`}>
        ...
        <div className="px-2 py-1 bg-blue-50 rounded border border-blue-200">
          ...
          <span className="ml-1 font-semibold text-blue-900">{statistics.loops}</span>
        </div>
        <div className="px-2 py-1 bg-purple-50 rounded border border-purple-200">
          <span className="text-purple-600">Conditionals:</span>
          <span className="ml-1 font-semibold text-purple-900">{statistics.conditionals}</span>
        </div>
        ...
        {validation && (
          <div
            ...
            <span className="font-semibold">{hasErrors ? "Errors" : hasWarnings ? "Warnings" : "Valid"}</span>
          </div>
        )}
      </div>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-2">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-xs font-semibold text-red-900 mb-1">Validation Errors</h4>
      ...
      {hasWarnings && !hasErrors && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-2">
          <div className="flex items-start gap-2">
            <FiAlertTriangle size={16} className="text-yellow-600 mt-0.5 flex-shrink-0" />
            <div className="flex-1 min-w-0">

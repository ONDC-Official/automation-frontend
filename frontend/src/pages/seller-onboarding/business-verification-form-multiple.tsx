...
const BusinessVerificationForm = ({
    ...
    const onSubmitForm = (data: BusinessVerificationFormData) => {
        ...
        const hasValidStore = data.stores.some((store: StoreDetails) => {
            ...
            if (isFnBDomain) {
                return (
                    baseFieldsValid && store.fssai_no && hasValidTimings && additionalFieldsValid
                );
            }
                    ...
                    <div>
                        ...
                        <Controller
                            ...
                            rules={{
                                ...
                                pattern: {
                                    value: /^-?([0-8]?[0-9]|90)(\.[0-9]{1,8})?,\s*-?((1[0-7][0-9]|[0-9]?[0-9])(\.[0-9]{1,8})?|180(\.0{1,8})?)$/,
                                    message:
        ...
        <form onSubmit={handleSubmit(onSubmitForm, onFormError)} className="space-y-8">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <p className="text-sm text-blue-800">
                    ...
                    {fields.length > 1 ? "s" : ""} configured. Each store will operate independently
                    with its own timings and serviceability settings.
                </p>
            </div>
        ...
        <div className="space-y-4">
                                                ...
                                                const duplicateExists = watchServiceabilities?.some(
                                                    (srv: Serviceability, idx: number) => {
                                                        ...
                                                        return (
                                                            ...
                                                            srv.category === value &&
                                                            srv.type ===
                                                                currentServiceability.type &&
                                                            srv.val === currentServiceability.val &&
                                                            (srv.type === "10"
                                                                ? srv.unit ===
                                                                  currentServiceability.unit
                                                                : true)
                                                        );
                                                    }
                                                );
                                                ...
                                                noDuplicates: (value: string | undefined) => {
                                                    ...
                                                    const duplicateExists =
                                                        watchServiceabilities?.some(
                                                            (srv: Serviceability, idx: number) => {
                                                                if (idx === serviceIndex)
                                                                    return false; // Skip current item
            ...
            {supportedSubcategories.length === 0 && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm mb-4">
                    <p className="text-yellow-800">
                        <strong>Important:</strong> No supported subcategories have been selected
                        for this store. Please select supported subcategories in the Additional
                        Details section above before configuring serviceability.
                    </p>
                </div>
            )}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm">
                <p className="text-blue-800">
                    <strong>Note:</strong> Serviceability defines where this store can deliver for
                    each category:
                </p>

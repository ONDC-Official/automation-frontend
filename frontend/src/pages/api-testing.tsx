...
const ApiTesting = () => {
    ...
    axios
      ...
      .then(async (response: any) => {
        if (npType === "BAP") {
          ...
          if (transactionId) {
            const transactionData = await getTransactionData(transactionId, subUrl);
            ...
            const completePayload = await getCompletePayload([currentPayloadID]);

            setResponseValue(JSON.stringify(completePayload[0].req || {}, null, 2));
          ...
          transactionIntervalRed.current = setInterval(async () => {
            ...
            transactionData?.apiList?.map(async (payload: any) => {
              if (payload.action === `on_${action}`) {
                ...
                const completePayload = await getCompletePayload([payload.payloadId]);
      ...
      .catch((e: any) => {
        console.error("something went wrong while fetching session: ", e);
        toast.error("Something went wrong while fetching session");
      });
  }

  const getAvailableActions = async (transcation_id: string, sessionId?: string) => {
    try {
      toast.info("Getting available actions.");
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/unit/safe-actions`, {
        params: {
      ...
      console.error("Error while getting actions", e);
      toast.error("Something went wrong while getting actions");
    }
  };

  const getPayload = async (action_id: string, session_id: string, filteredAction?: string) => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/unit/trigger/${filteredAction || action}`, {
        params: {
  ...
  const sendPayload = async (data?: any, action_id?: string) => {
    ...
    try {
      setIsLoading(true);
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/unit/trigger/${action}`, body, {
        params: {
          transaction_id: cuurentTranscationId,
          subscriber_url: subUrl,
  ...
  return (
    <div className={`w-[100%] mt-1 shadow-md flex flex-col transition-all duration-300 overflow-y-scroll`}>
      ...
      <div className="w-[100%] flex flex-row">
        <div className="w-3/6 p-4 gap-4 flex flex-col">
          <div className="flex flex-row items-center justify-between">
            <div className="flex flex-row items-center gap-4">
              <h1 className="text-lg font-semibold text-gray-800">{npType === "BAP" ? "On Action" : "Action"}</h1>
              <div className="flex flex-row items-center cursor-pointer group" onClick={() => setIsModalOpen(true)}>
          ...
          <div className="flex flex-col">
            ...
            <div className={`relative mt-4 flex`}>
              <div
                className={`absolute right-5 top-2 z-10 ${!isEdittable ? "bg-blue-200" : "bg-gray-200"} p-2 rounded-md`}
        ...
        <div className="w-3/6 flex flex-col gap-4 my-4 ">
          <div className="flex flex-row justify-between">
            <h1 className="text-lg font-semibold text-gray-800">{npType === "BAP" ? "Action" : "On Action"}</h1>
          </div>
          <div className="h-[47%]">
            <Editor
              theme="vs"
              ...
              <Markdown
                ...
                components={{
                  ...
                  code: ({ inline, children }: any) =>
                    inline ? (
                      <code className="bg-gray-100 text-red-600 rounded px-1">{children}</code>
                    ) : (

...
export const patch = async (data: any, user?: UserDetails) => {
	try {
		...
		console.log("Response from patch request:", res.data);
	} catch (error) {
		if (axios.isAxiosError(error)) {
			toast.error(`Error: ${error.response?.data?.details || error.message}`);
		}
		...
		const convertedData: SubscriberData = {
			...
			mappings: data.mappings.map((mapping: any) => {
				return {
					...
					location_country:
						locations.find((l: any) => l.id === mapping.location_id)
							?.country[0] || "",
					location_city: locations.find(
						(l: any) => l.id === mapping.location_id
					)?.city || ["*"],
				};
			}),
		};
		// console.log("Converted subscriber data:", convertedData);
